<template>
  <div>
    <el-row v-if="$hasAnyAuthority(['PROJECT_ADMIN_' + projectId, 'PROJECT_MASTER_' + projectId])">
      <el-col :span='24'>
        <el-button type='primary'
                   class='float-right'
                   size="mini"
                   @click="newMember()">添加成员</el-button>
      </el-col>
    </el-row>
    <el-row>
      <el-table v-loading="loading"
        ...
        <el-table-column label="操作"
          ...
          <template slot-scope="scope"
                    v-if="project && scope.row.username !== project.chargedBy && name !== scope.row.username">
            <el-button type="text"
                       @click="assignCenter(scope.row)">分配中心</el-button>
            <el-divider direction="vertical"></el-divider>
            <el-button type="text"
                       @click="assignTask(scope.row)">分配角色</el-button>
            <el-divider direction="vertical"></el-divider>
            <el-button type="text"
    ...
    <el-dialog v-if="deleteMemberDialogVisible"
               title="确认删除成员"
               width="500px"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   :disabled="isSaving"
                   @click="confirmDelete">删 除</el-button>
      </span>
    </el-dialog>
